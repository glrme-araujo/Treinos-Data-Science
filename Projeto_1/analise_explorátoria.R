...
dataset <- dataset %>%
  gather('Larg.Petala','Larg.Sepala', key = "Largura", value = 'Larg.val')%>%
  gather('Compr.Petala','Compr.Sepala', key = "Comprimento", value = 'Compr.val')
...
dataset <- dataset %>% relocate(Espécie, .after= Observação)%>%
                       ...
                       relocate(Unidade, .after= Compr.val)
            
# Renomeando Colunas
dataset <- rename(dataset,
        Comprimento = Compr.val,
        Largura = Larg.val
       )
...
descritiva <- descritiva%>% group_by(Parte_floral)%>%
  arrange(.by_group = TRUE)
